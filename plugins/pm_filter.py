...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AREBHAI, ADPIC, HLPIC, VERIFIED, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            return await message.reply_text(f"<b>Hello 👋🏻 {message.from_user.mention}, {str(total_results)} Search Results for your query 🔎 {search}. \n\nSorry 💔 Get in the <a href='https://t.me/yedekho'>YeDekho Platforms 🔎</a> to Search up your content...\n\nHop into the Search Engine-@yedekho</b>")
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>माफ़ करिये {user} यहाँ पे कुछ नहीं मिलेगा\nयहाँ अनुरोध करो 👉🏻 @yedekho 🔍\n<b>केवल फिल्म का नाम और वर्ष लिखें</b>\n\n𝖲𝗈𝗋𝗋𝗒 {user} 𝖻𝗎𝗍 𝗒𝗈𝗎 𝖼𝖺𝗇'𝗍 𝗀𝖾𝗍 𝖺𝗇𝗒𝗍𝗁𝗂𝗇𝗀 𝗁𝖾𝗋𝖾\n𝗋𝖾𝗊𝗎𝖾𝗌𝗍 𝗁𝖾𝗋𝖾 👉🏻 @yedekho 🔍\n<b>T𝗒𝗉𝖾 𝗈𝗇𝗅𝗒 𝗆𝗈𝗏𝗂𝖾 𝗇𝖺𝗆𝖾 & 𝗒𝖾𝖺𝗋</b>",   
...
async def next_page(bot, query):
    ...
    #     ]
        ...
        btn.insert(0, [
            InlineKeyboardButton("🤔 How To download ?", url=f"https://telegram.me/yedekho_faq/25")
        ])
    else:
        btn = []
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("Results", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("Next", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("Results", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("Next", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("🏃 Back", callback_data=f"next_{req}_{key}_{off_set}"),
        ...
        if n_offset == 0:
            ...
            btn.append([InlineKeyboardButton("Result", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("Next", callback_data=f"next_{req}_{key}_{n_offset}")])
        else:
            btn.append(
                [
                    InlineKeyboardButton("🏃 Back", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="𝖲𝖾𝗅𝖾𝖼𝗍 𝖸𝗈𝗎𝗋 preffered 𝖫𝖺𝗇𝗀𝗎𝖺𝗀𝖾", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="Back to results 🔍", callback_data=f"fl#homepage#{key}")])
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("Results", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="Next",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("Results", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="Next",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
        ...
        btn.append(
            [InlineKeyboardButton(text="Thats The End 🔚",callback_data="pages")]
        )
    # if ENABLE_SHORTLINK == True:
    btn.insert(0, [
        InlineKeyboardButton("🤔 How to Download", url=f"https://telegram.me/yedekho_faq/25")
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="𝖲𝖾𝗅𝖾𝖼𝗍 the Season", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="Back to Results 🔙", callback_data=f"next_{req}_{key}_{offset}")])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                if clicked == typed:
                    ...
                    await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
                await query.answer('Check your personal message ✅', show_alert=True)
        ...
        files_ = await get_file_details(file_id)
        if not files_:
            return await query.answer('That File doesnt exist now.')
        files = files_[0]
        title = files.file_name
        size = get_size(files.file_size)
        f_caption = files.caption
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Seriously 😑 Firstly join the channel", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        if settings is not None:
            ...
            await client.send_message(
                chat_id=userid,
                text=f"<b>Change the Settings for {title} as you want it to be ⚙️</b>",
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/yedekho")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            InlineKeyboardButton('🤌🏻 Okay i understood', callback_data='filters')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton("Downloading & Playing 🎯", url="https://t.me/yedekho_faq/25")
        ], [
        ...
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto(random.choice(HLPIC))
        )
        await query.message.edit_text(
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Use Buttons', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await client.edit_message_media(
            query.message.chat.id, 
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Donate - 9 Rs', url="https://bit.ly/yedekho-donate-9rs"),
            InlineKeyboardButton('Donate - 29 Rs', url="https://bit.ly/yedekho-donate-29rs")
        ],[
            InlineKeyboardButton('Donate - 49 Rs ', url="https://bit.ly/yedekho-donate-49rs"),
            InlineKeyboardButton('Donate - 99 Rs', url="https://bit.ly/yedekho-donate-99rs")
        ],[
            InlineKeyboardButton('I am Cruel 🙁, I will not Donate 🤡', callback_data='start')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto(random.choice(AREBHAI))
        )
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto(random.choice(ADPIC))
        )
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('🤌🏻 Okay i Understood', callback_data='manuelfilter')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton('🏃 Back', callback_data='help'),
            InlineKeyboardButton('✅ Verify', callback_data='shortlink_info')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton('🏃 Go Back', callback_data='extra')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton('🏃 Go Back', callback_data='help'),
            InlineKeyboardButton('🔁 Refresh - F5', callback_data='rfrsh')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    # else:
    if settings["button"]:
        cap = f"<b>🔍 Search Results for {search}\n\n💛 Requested by: {message.from_user.mention}\n\n⏳ Showing Results in : {remaining_seconds} Seconds\n\n⭐ Powered by : {message.chat.title} \n\n ⚠️ Note - The links would soon be deleted automatically 🗑️\n\n</b>"
    else:
        # cap = f"<b>Hᴇʏ {message.from_user.mention}, Hᴇʀᴇ ɪs ᴛʜᴇ ʀᴇsᴜʟᴛ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {search} \n\n</b>"
        cap = f"<b>Hey {message.from_user.mention}, Found {total_results} results for your query to {search}\n\n</b>"
        for file in files:
            cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b>"
